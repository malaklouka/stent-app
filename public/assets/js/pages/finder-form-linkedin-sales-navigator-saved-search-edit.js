...
stent.finders.salesNavigator = (function () {
  ...
  const readOnlyForm = function () {
    $("#finder-retrieve-from").prop("disabled", true);
    ...
    $("#excludeViewed").prop("disabled", true);
    $(".finder-mapping-field").prop("disabled", true);
    $("#finder-query-definition .card").css("background-color", "#fbf9fd");
  ...
  const fillForm = async function () {
    ...
    if (_finder) {
      ...
      } else if (_finder.searchName && _finder.searchName !== "") {
        $("#finder-name").val(_finder.searchName);
      }
      ...
      if (_finder.provisioner && _finder.provisioner.filter && _finder.provisioner.filter.company && _finder.provisioner.filter.company.id) {
        ...
        if (_segmentsAndFindersCompanies) {
          populateFilterOnCompanySelect(_finder.provisioner.filter.company.id);
        }
  ...
  const save = async function () {
    ...
    let sendFinder = await stent.finders.form.putFinder(finder);
