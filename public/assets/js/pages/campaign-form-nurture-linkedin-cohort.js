...
stent.requireJS(["flatpickr"], function () {
  stent.campaign.invite = (function () {
    let _campaignFlow = "nurture-linkedin-cohort";
    let _campaignId;
    let _campaignSource;
    ...
    let editorsConfiguration = [
      ...
      // INVITE
      {
        type: "invite",
        name: "Invite",
        delayBeforeReInvite: "PT0S",
        allowChooseChannel: false,
        channel: "linkedin-invite-message",
        channels: {
    ...
    const fillForm = function () {
      ...
      if (item) {
        ...
        $(".main-content h1").text(_isInDuplication ? "Duplicate an invite campaign" : "Edit an invite campaign");
    ...
    const checkForm = function () {
      ...
      // ------------------------------- //
      // If there is NO Nudges message
      // ------------------------------- //

      if (editors.filter((editor) => editor.type === "nudge").length === 0) {

        // Default message is not filled
        if (editors[0].body.default === "") {
          $("#campaign-tab-0").addClass("is-invalid").click();
          $("#editor-wrapper-0 .switch-language[data-language='default']").addClass("is-invalid").click();
          formHasError = true;
        }

        // Check also the variations of the default message
        if (editors[0].body.variations && editors[0].body.variations.length > 0) {
          for (let i = 0; i < editors[0].body.variations.length; i++) {
            let variation = editors[0].body.variations[i];
            let lang = variation.lang;

            if ($.trim(variation.body) === "") {
              $("#editor-wrapper-0 .switch-language[data-language='" + lang + "']").addClass("is-invalid").click();
        ...
        // Greeting message becomes mandatory, if the user added some variations and filled at least one
        if (editors[1].body.default === "" && editors[1].body.variations && editors[1].body.variations.length > 0) {
          // check if at least one variation is filled, then default and all variations are mandatory
          let concatVariations = editors[1].body.variations
            .map((variation) => {
              return Object.values(variation)[0];
            })
            .join("");

          if (concatVariations !== "") {
            $("#campaign-tab-1").addClass("is-invalid").click();
            $("#editor-wrapper-1 .switch-language[data-language='default']").addClass("is-invalid");

            for (let i = 0; i < editors[1].body.variations.length; i++) {
              let variation = editors[1].body.variations[i];
              let lang = variation.lang;

              if ($.trim(variation.body) === "") {
                $("#editor-wrapper-1 .switch-language[data-language='" + lang + "']").addClass("is-invalid").click();
        ...
        // There is at least one nudge, then all the editors and all the variations are mandatory.
    ...
    const onAfterSave = function () {
      stent.loader.hide();
      stent.ui.pushState("campaign-list-nurture-linkedin-cohort", false, "campaign-list-nurture-linkedin-cohort");
      stent.ui.load({ fileToLoad: "campaign-list-nurture-linkedin-cohort" });
    };
    ...
    const init = async function () {
      ...
      // change Page title
      stent.ui.setPageTitle("Invite campaign creation / edition");
