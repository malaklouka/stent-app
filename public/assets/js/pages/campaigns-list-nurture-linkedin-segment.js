...
stent.campaign.list.segment = (function () {

  let _flow = "nurture-linkedin-segment";
  let _tableCols = [2, 2, 1, 1, 1, 1, 1, 1, 1, 1];
  let _members = null;
  let _campaigns = null;
  let _programs = null;
  let _filters = {
  ...
  const funelLink = `${urlOrigin + "/" + firstPath + "/funnel"}`;
        ...
        <table class="table table-vcenter">
          <thead>
            <tr class="d-flex">
              ...
              <th class="col-${_tableCols[5]} text-right">SENT</th>
              <th class="col-${_tableCols[6]} text-right">CLICKS</th>
              <th class="col-${_tableCols[7]} text-right">REPLIES</th>
              <th class="col-${_tableCols[9]} text-center">STATUS</th>
              <th class="col-${_tableCols[9]} text-right">ACTIONS</th>
            </tr>
          </thead>
          <tbody></tbody>
        </table>
  ...
  const programDOM = function (program) {
    ...
    /* eslint-disable */
    html += `<tr class="d-flex lv-0" data-program-name="${programName}">`;
    html += `
      <!-- Program Name -->
      ...
      ${!$("#showArchived").is(":checked") ?
        ...
        <div class="dropdown-menu dropdown-menu-right dropdown-toggle-no-caret">
          <a href="queue?program=${encodeURIComponent(campaigns[0].program)}&sort=DATE_DESC" class="dropdown-item ui-link">
           ...
           <a class="dropdown-item ui-link" onclick="window.open('${funelLink}?program=${encodeURIComponent(campaigns[0].program)}', ''); return false"  href="javascript:void(0);">
             <i class="fe fe-arrow-up-right"></i> View funnel
            </a>
        </div>
        ` :
        ""
  ...
  const campaignDOM = function (campaign, index, campaignsLength) {
    ...
    let endDate = campaign.schedule.endDate;
    let checkBefore = "";
    if (endDate) {
      checkBefore = moment().isBefore(endDate, "day");
    }
    ...
    if (campaign.health && campaign.health.status) {
      ...
      } else if (campaign.health.status === "stopped" && campaign.status === "active") {
        health = "stopped";
        healthText = "stopped";
        healthTooltip = "<strong>Campaign is stopped.</strong>" + stent.utils.arrayToHMLList(campaign.health.errors);
    ...
    let endedDisplay = `
      <img src="/assets/img/campaigns/stopped.svg" class="health-icon mr-2" />
    ...
    if (
      ...
      typeof campaign.health.nurtures !== "undefined" &&
      typeof campaign.health.slots !== "undefined" &&
      typeof campaign.health.slots.scheduled !== "undefined") {

      let current = 0;
      current += campaign.health.nurtures.blocked;
      current += campaign.health.nurtures.duplicates;
      current += campaign.health.nurtures.errors;
      current += campaign.health.nurtures.falsy;
      current += campaign.health.nurtures.sent;

      let totalDay = campaign.health.nurtures.total ?
        campaign.health.nurtures.total :
        campaign.health.nurtures.cohort.size * campaign.health.slots.scheduled;
      ...
      progressTooltip = `
        <ul style='text-align: left; margin: 0; padding: 0 0 0 15px;'>
          <li class='${campaign.health.nurtures.blocked > 0 ? "font-weight-bold" : ""}'>${campaign.health.nurtures.blocked} blocked</li>
          <li class='${campaign.health.nurtures.duplicates > 0 ? "font-weight-bold" : ""}'>${campaign.health.nurtures.duplicates} duplicate${campaign.health.nurtures.duplicates > 1 ? "s" : ""}</li>
          <li class='${campaign.health.nurtures.errors > 0 ? "font-weight-bold" : ""}'>${campaign.health.nurtures.errors} error${campaign.health.nurtures.errors > 1 ? "s" : ""}</li>
          <li class='${campaign.health.nurtures.falsy > 0 ? "font-weight-bold" : ""}'>${campaign.health.nurtures.falsy} falsy</li>
          <li class='${campaign.health.nurtures.sent > 0 ? "font-weight-bold" : ""}'>${campaign.health.nurtures.sent} sent</li>
        </ul>
        <div style='margin-top: 5px; padding-top: 5px; border-top: 1px solid #c4b3dd;'>On a total of ${totalDay} message${totalDay > 1 ? "s" : ""}</div>
      `;
    }
    ...
    const targetIconDOM = function (target) {
      ...
      return `<b class="target-icon mr-1 target-icon-${_target.toLowerCase()} text-capitalize">${_target.charAt(0)}</b>`;
    };
    ...
    html += `
        ...
        <span
            ...
            Target: <strong>${campaign.target}</strong> <br/></div>">
            ${targetIconDOM(campaign.target)}
          </span>
        ...
        <td class="col-${_tableCols[2]} text-break pr-5">
          <a 
            ...
            href="${campaign.source.type === "finders" ? "finder-form" : "segments-edit"}?id=${campaign.source.key}">
              <span class="fe fe-${campaign.source.type === "finders" ? `search` : `taget`} mr-1"></span>${campaign.source.name}</a>
        </td>
        ...
        <td class="col-${_tableCols[3]}">
          <div 
            ...
            data-delay="100" 
            data-html="true" 
            title="<div style='font-size: 11px;'>${checkBefore === false ? `The campaign has ended.` : `${healthTooltip}`}</div>">
              <span>${checkBefore === false ? `${endedDisplay}` : `${heathDisplay}`}</span>
            </div>
        </td>
        ...
        <td class="col-${_tableCols[5]} align-right">
            ${campaign.sent && campaign.sent > 0 ? campaign.sent.toLocaleString(stent.locale) : '-'}
        </td>
        ...
        <td class="col-${_tableCols[6]} align-right">
          <div
            ...
            data-delay="100" 
            data-html="true" 
            title="<div style='font-size: 11px;'>${campaign.clicks && campaign.clicks > 0 ?
        ((campaign.clicks * 100) / campaign.sent).toFixed(0) + "%" :
        "No click"
      }</div>">
            ${campaign.clicks && campaign.clicks > 0 ? campaign.clicks.toLocaleString(stent.locale) : '-'}
          </div>
        </td>

        <!-- Replies -->
        <td class="col-${_tableCols[7]} justify-content-end">
          ${(
        campaign.replies && campaign.replies.count && campaign.replies.count > 0 ?
          `<span 
                data-toggle="tooltip"
                data-delay="100" 
                data-html="true" 
                title="<div style='font-size: 11px;'>${((campaign.replies.count * 100) / campaign.sent).toFixed(0) + "%"}</div>">`
          + campaign.replies.count.toLocaleString(stent.locale) +
          `</span>`
          :
          `-`
      )
      ...
      (
        campaign.replies &&
          campaign.replies.count &&
          campaign.replies.count > 0 &&
          typeof campaign.replies.sentiment !== "undefined" && campaign.replies.sentiment > 0
          ? `<img 
                      class="ml-2" 
                      src="/assets/img/dashboard/sentiment-${campaign.replies.sentiment >= stent.breakpoints.sentimentAnalysis ? `up` : `down`}.svg" 
                      data-toggle="tooltip"
                      data-delay="100" 
                      data-html="true" 
                      title="<div style='font-size: 11px;'>${campaign.replies && campaign.replies.sentiment > 0 ?
            "Sentiment: " + Math.round(campaign.replies.sentiment * 100) + "%" :
            "No reply"
          }</div>"
                      width="18" />`
          : `<img class="ml-2" src="/assets/img/magic.png" width="18" />`
      )
      }
        </td>
        ...
        <!-- Status -->
        <td class="col-${_tableCols[8]} justify-content-center">
          ${!$('#showArchived').is(':checked') ?
        `
        ...
        <!-- Actions -->
        <!-- Actions -->
      <td class="col-${_tableCols[9]} align-right">
        ${$('#showArchived').is(':checked') ?
        ...
        `
          ...
          <div class="dropdown-menu dropdown-menu-right dropdown-toggle-no-caret">
            ...
            <a 
             ...
             <!-- Go to Funnnel -->
            <a class="dropdown-item ui-link" onclick="window.open('${funelLink}?campaignName=${encodeURIComponent(campaign.name)}&ambassador=${encodeURIComponent(campaign.identityKey)}&program=${encodeURIComponent(campaign.program)}', ''); return false"  href="javascript:void(0);">
             <i class="fe fe-arrow-up-right"></i> View funnel
            </a>

            <a href="queue?campaignId=${campaign._key}&sort=DATE_DESC" class="dropdown-item ui-link">
              <i class="fe fe-users"></i> View queue
            </a>
  ...
  const init = async function () {
    ...
    stent.navbar.activeMenu("campaign-list-nurture-linkedin-segment");
    // change Page title
    stent.ui.setPageTitle("Nurture campaigns list");
