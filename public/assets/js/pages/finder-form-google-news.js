...
stent.finders.googleNews = (function () {
  ...
  const readOnlyForm = function () {
    ...
    $("#finder-keywords").prop("disabled", true);
    $("#finder-news-type").prop("disabled", true);
    $("#finder-language").prop("disabled", true);
    $("#finder-country").prop("disabled", true);
  ...
  const fillForm = function () {
    ...
    if (_finder) {
      ...
      // keywords
      if (_finder.provisioner && _finder.provisioner.query && _finder.provisioner.query.keywords && _finder.provisioner.query.keywords !== "") {
        $("#finder-keywords").val(_finder.provisioner.query.keywords);
      }

      // news type
      if (_finder.provisioner && _finder.provisioner.query && _finder.provisioner.query.scope && _finder.provisioner.query.scope !== "") {
        $("#finder-news-type option[value='" + _finder.provisioner.query.scope + "']").prop("selected", true);
      }

      // finder-language
      if (_finder.provisioner && _finder.provisioner.query && _finder.provisioner.query.language && _finder.provisioner.query.language !== "") {
        $("#finder-language option[value='" + _finder.provisioner.query.language + "']").prop("selected", true);
      }

      // finder-country
      if (_finder.provisioner && _finder.provisioner.query && _finder.provisioner.query.country && _finder.provisioner.query.country !== "") {
        $("#finder-country option[value='" + _finder.provisioner.query.country + "']").prop("selected", true);
      }
      ...
      // email lookup
      if (_output === "contact" && typeof _finder.emailLookup !== "undefined" && _finder.emailLookup !== null && _finder.emailLookup === true) {
  ...
  const checkForm = function () {
    ...
    // Check finder-keywords
    if ($.trim($("#finder-keywords").val()) === "") {
      errors.push({
        id: "finder-keywords",
  ...
  const save = async function () {
    ...
    let finder = {
      ...
      provisioner: {
        ...
        query: {
          ...
          language: $("#finder-language").val(),
          country: $("#finder-country").val(),
          scope: $("#finder-news-type").val()
        }
      },
      schedule: {
    ...
    // Email lookup
    if (_output === "contact") {
      finder.emailLookup = $("#emailLookup").is(":checked");
    }
  ...
  const init = async function () {
    ...
    // Email lookup
    if ($("#finder-output").val() === "contact") {
      _output = "contact";
      $("#emailLookup-wrapper").removeClass("d-none");
    } else {
      _output = "company";
    }
