...
stent.finders.linkedInSuggestedContacts = (function () {
  ...
  const fillForm = function () {
    ...
    if (_finder) {
      ...
      // reason context
      if (_finder.provisioner && typeof _finder.provisioner.reasonContext !== "undefined" && _finder.provisioner.reasonContext !== null) {
        $("#reasonContext_" + _finder.provisioner.reasonContext).prop("checked",true);
      }

      // Readonly the reason context
      $("[name='reasonContext']").prop("disabled", true);
  ...
  const checkForm = function () {
    ...
    if (typeof $("[name='reasonContext']:checked").val() === "undefined") {
      errors.push({
        selector: "#reasonContext-wrapper .card",
        message: "Please choose a reason context."
      });
      formHasError = true;
    }
  ...
  const save = async function () {
    ...
    let finder = {
      ...
      provisioner: {
        ...
        reasonContext: $("[name='reasonContext']:checked").val(),
      },
      schedule: {
