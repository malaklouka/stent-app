...
stent.requireJS(["flatpickr"], function () {
  stent.campaign.nurture = (function () {
    let _campaignFlow = "nurture-linkedin-segment";
    let _campaignId;
    let _campaignSource;
    ...
    const sourcesURLFunction = function () {
      return "/tenants/" + stent.tenant.key + "/segments/owners/" + $("#campaign-sender").val();
    };

    const populateSourcePicker = function (items) {
      $("#campaign-source").empty();
      if (!items || (items && items.length == 0)) {
        let option = new Option("Please select a valid sender having segments !", "");
    ...
    const fillForm = function () {
      ...
      if (item) {
        ...
        _campaignSource =
          item.parameters && item.parameters.segment && item.parameters.segment.id ? item.parameters.segment.id : null;

        $("#campaign-form").toggleClass("d-none");
        $(".main-content h1").text(_isInDuplication ? "Duplicate a nurture campaign" : "Edit a nurture campaign");
    ...
    const checkForm = function () {
      ...
      if ($.trim($("#campaign-source").val()) === "") {
        errors.push({
          id: "campaign-source",
          message: "Please choose a segment.",
        });
        formHasError = true;
      }
    ...
    const save = function () {
      ...
      if (_originalCampaign &&
          ...
          !_isInDuplication ) {
        campaign.parameters.source = {
          queueId: _originalCampaign.parameters.source.queueId
        };
      }
    ...
    const onAfterSave = function () {
      stent.loader.hide();
      stent.ui.pushState("campaign-list-nurture-linkedin-segment", false, "campaign-list-nurture-linkedin-segment");
      stent.ui.load({ fileToLoad: "campaign-list-nurture-linkedin-segment" });
    };
    ...
    const init = async function () {
      ...
      // change Page title
      stent.ui.setPageTitle("Nurture campaign creation / edition");
