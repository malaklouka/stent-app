...
stent.planificationMobile = (function () {
    console.log("====================================");
    console.log("Tenant Edit corporate.js");
    console.log("====================================");
    ...
    let members = [{}];
    let filterPage = "";
    let selectedIdToCorporatePage = "";
    let selectedPageDuplicate = "";
    ...
    // *
    const getEditCorporateData = async function () {
        ...
        if (result && result.message && result.message.data && result.message.data.workspaceContext && result.message.data.workspaceContext.contentScheduleById) {
            ...
            // *
            postCreation = {
                channelId: knowChannel == "LinkedInPageContentSchedule" ? "linkedin-corporate-page" : "linkedin-personal-page",
            ...
            let acutalDate = result.message.data.workspaceContext.contentScheduleById.schedule;
    ...
    const getPublishers = async function () {
        ...
        let query = `query {
                      workspaceContext {
                        members(where: {type: MEMBER channel: LINKEDIN status: ACTIVE},first: ${initialPublishers}) {
                        edges {
                            node {
                            type: __typename
                            ... on Node {
    ...
    const populatePublishers = function () {
        ...
        let optionsPublishers = _publishers.filter(({ node: publisher }) => {
            return publisher.accounts.length && publisher.accounts[0].active;
        }).map(({ node: publisher }) => {
        ...
        // Filter and add value to selected mebmer
        ...
        $("#publisherShow").on("select2:select", function () {
            ...
            $("#pageSelector").empty().append(new Option("Select a corporate page", "0"));
            getCorporatePage();
        });
    ...
    // Get Corpotate Page
    const getCorporatePage = async function () {
        let selectedId = members.map(({ id }) => ({
            id
        }));

        let IDCorporatePages = selectedId[0].id;

        let query = `
                    {
                        workspaceContext {
                            accountsByMemberId(memberId: "${IDCorporatePages}") {
                            ... on LinkedInAccount {
                                firstName
                                lastName
                                adminCorporatePages {
        ...
        let selectedDuplicateItemPage = postCreation.timeline[0].origin.name;

        if (result && result.message && result.message.data && result.message.data.workspaceContext.accountsByMemberId && result.message.data.workspaceContext.accountsByMemberId[0] && result.message.data.workspaceContext.accountsByMemberId[0].adminCorporatePages) {
            let $pageSelector = $("#pageSelector");
            let $noPageSelector = $("#noPageSelector");
            let corporatePagesResult = result.message.data.workspaceContext.accountsByMemberId[0].adminCorporatePages;


            if (corporatePagesResult.length >= 1) {
                $pageSelector.addClass("d-block").removeClass("d-none");
                $noPageSelector.addClass("d-none").removeClass("d-block");
                $("#goToTimeRange").removeAttr("disabled");

                $.each(corporatePagesResult, (i, item) => {
                    let selectedOption = selectedDuplicateItemPage == `${item.name}` ? "selected" : "";

                    $pageSelector.append(function (i) {
                        return `                            
                            <option value="${item.id}" data-id="${item.id}" data-name="${item.name}" ${selectedOption}>${item.name}</option>
                        `;
                    });
                });
            } else {
                $("#goToTimeRange").attr("disabled", "disabled");
                $noPageSelector.removeClass("d-none").addClass("d-block");
                $pageSelector.addClass("d-none").removeClass("d-block");
            }
    ...
    const scheduleContent = async function () {
        ...
        let query = `
            mutation {
                workspaceContext {
                    updateLinkedInPageShareContentSchedule(
                    id: "${getScheduleID}"
                    input: {
                        mediaId: "${mediaId}"
                        doNotResend: ${resend}
    ...
    const bindEvents = function () {
        ...
        // *
        $(".resetScheduleElements").on("click", function () {
        ...
        $("#goToTimeRange")
            ...
            .on("click", function () {
                ...
                var id = $("#pageSelector").find(":selected").attr("data-id");
                var name = $("#pageSelector").find(":selected").attr("data-name");
                corporatePageId = id;
                corporatePageName = name;
            });
