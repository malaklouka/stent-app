...
stent.campaign.list.cohort = (function () {

  let _flow = "nurture-linkedin-cohort";
  ...
  const campaignDOM = function (campaign, index, campaignsLength) {
    ...
    if (
      ...
      typeof campaign.health.invites !== "undefined" &&
      typeof campaign.health.slots !== "undefined" &&
      typeof campaign.health.slots.scheduled !== "undefined") {

      let current = 0;
      current += campaign.health.invites.blocked;
      current += campaign.health.invites.duplicates;
      current += campaign.health.invites.errors;
      current += campaign.health.invites.falsy;
      current += campaign.health.invites.pending;
      current += campaign.health.invites.sent;

      let totalDay = campaign.health.invites.total ?
        campaign.health.invites.total :
        campaign.health.invites.cohort.size * campaign.health.slots.scheduled;
      ...
      progressTooltip = `
        <ul style='text-align: left; margin: 0; padding: 0 0 0 15px;'>
          <li class='${campaign.health.invites.blocked > 0 ? "font-weight-bold" : ""}'>${campaign.health.invites.blocked} blocked</li>
          <li class='${campaign.health.invites.duplicates > 0 ? "font-weight-bold" : ""}'>${campaign.health.invites.duplicates} duplicate${campaign.health.invites.duplicates > 1 ? "s" : ""}</li>
          <li class='${campaign.health.invites.errors > 0 ? "font-weight-bold" : ""}'>${campaign.health.invites.errors} error${campaign.health.invites.errors > 1 ? "s" : ""}</li>
          <li class='${campaign.health.invites.falsy > 0 ? "font-weight-bold" : ""}'>${campaign.health.invites.falsy} falsy</li>
          <li class='${campaign.health.invites.pending > 0 ? "font-weight-bold" : ""}'>${campaign.health.invites.pending} pending${campaign.health.invites.errors > 1 ? "s" : ""}</li>
          <li class='${campaign.health.invites.sent > 0 ? "font-weight-bold" : ""}'>${campaign.health.invites.sent} sent</li>
        </ul>
        <div style='margin-top: 5px; padding-top: 5px; border-top: 1px solid #c4b3dd;'>On a total of ${totalDay} invite${totalDay > 1 ? "s" : ""}</div>
      `;
    }
  ...
  const ratioDOM = function (acceptance) {
    ...
    if (acceptance > 0 && acceptance < 20) {
      return `
    < div class="battery-jauge battery-jauge_danger" ></ >
  ...
  `;
      ...
      return `
    < div class="battery-jauge battery-jauge_success" ></ >
  ...
  const init = async function () {
    ...
    stent.navbar.activeMenu("campaign-list-nurture-linkedin-cohort");
    // change Page title
    stent.ui.setPageTitle("Invite campaigns list");
